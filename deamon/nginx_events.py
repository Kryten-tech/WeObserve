...
class Events(watchdog.events.PatternMatchingEventHandler): 
    ...
    def on_created(self, event):
        if event.src_path[-3:] in self.fucky_endings:
            return
        c_notification = "Received creation event - % s - % s - % s" % (event.src_path, time.ctime(os.path.getctime(event.src_path)), IPAddr)
        # print(c_notification) #Prints immediately
        print(c_notification, file=open("/var/log/kryten/nginx/customer.txt", "a")) # Prints into specified txt file
    ...
    def on_modified(self, event):
        ...
        print(m_notification, file=open("/var/log/kryten/nginx/customer.txt", "a"))

        # File access event:
    def on_deleted(self, event):
        if event.src_path[-3:] in self.fucky_endings:
            return
        # d_notification = "File/Directory was deleted - % s - % s - % s" % (event.src_path, time.ctime(os.path.getctime(event.src_path)), IPAddr)
        d_notification = "File/Directory was deleted - % s - % s - % s" % (event.src_path, datetime.datetime.now(), IPAddr)
        # print(d_notification)
        print(d_notification, file=open("/var/log/kryten/nginx/customer.txt", "a"))
        

if __name__ == "__main__":
    src_path = r"/var/www/" # /path/to/directory or file
